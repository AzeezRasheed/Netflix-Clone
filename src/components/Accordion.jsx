   ...
   <div>
    ...
    {datas.map((data) => (
        <div key={data.id} className='item'>
          <h3 className='header' onClick={() => toggle(data.id)}>
          {data.header} {activeNav === data.id ? <AiOutlineMinus/> : <AiOutlinePlus/>}
          </h3>
          <p className={`body ${activeNav=== data.id ? 'show': ''}`}>
            {data.body}
          </p>
        </div>
      ))}
